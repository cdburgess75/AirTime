...
#ifdef AIRTIME
// ── The schedule ships inside the firmware ──────────────────────────────────
//
// Stock ats-mini downloads eibi.txt from the internet, and on this radio that
// can never happen: the WiFi is its own bare access point with no route
// anywhere, by design (PLAN.md §2 — the radio serves time; it does not browse).
// So the schedule text is compiled into the image (EiBiData.h, generated by
// tools/eibi_embed.py) and installed into LittleFS from there, through the
// SAME eibiParseLine() the download path uses — the embedded copy cannot
// diverge from the format the firmware expects, because there is only one
// parser and it is the arbiter of the format.
//
// The dataset's content hash rides along; when the installed hash matches,
// this returns before touching flash — every boot after the first costs a
...
bool eibiInstallEmbedded(bool force)
{
  ...
  if(!force && LittleFS.exists(EIBI_PATH))
  {
    char ver[32] = {0};
    fs::File vf = LittleFS.open(EIBI_VER_PATH, "rb");
    if(vf)
    {
      vf.read((uint8_t*)ver, sizeof(ver) - 1);
      vf.close();
    }
    if(!strcmp(ver, kEibiDataVersion)) return(true);
  }

  drawScreen(eibiMessage, "Installing embedded copy...");
  ...
  // The same scrub-and-parse loop as the download path, fed from the image.
  // The <= is deliberate: it manufactures a final newline so the last line of
  // the embedded text is flushed rather than silently dropped.
  const char *src = (const char*)kEibiText;
  char charBuf[200];
  size_t charCnt = 0;
  int lineCnt = 0;
  ...
  if(!lineCnt)
  {
    ...
    drawScreen(eibiMessage, "Embedded data held no entries!");
    return(false);
  }

  // Move new schedule to its permanent place, and remember whose it is.
  LittleFS.remove(EIBI_PATH);
  LittleFS.rename(TEMP_PATH, EIBI_PATH);
  fs::File vf = LittleFS.open(EIBI_VER_PATH, "wb");
  if(vf)
  {
    vf.write((const uint8_t*)kEibiDataVersion, strlen(kEibiDataVersion));
    vf.close();
  }

  identifyFrequency(currentFrequency + currentBFO / 1000);
  char doneMessage[64];
  sprintf(doneMessage, "DONE! %d entries (%s)", lineCnt, kEibiDataVersion);
  drawScreen(eibiMessage, doneMessage);
  return(true);
}
#endif // AIRTIME
