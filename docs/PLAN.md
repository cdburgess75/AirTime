...
- ~~**No WWV timecode (date) decoding.** The BCD timecode rides a 100 Hz subcarrier below the SI473x audio passband. WWV provides *phase only* (second/minute alignment). Date and minute number come from RDS, warm-boot RTC memory, or manual entry.~~
  **Reversed 2026-08-04.** This settled decision turned out to be the design's worst structural weakness at the actual QTH: with WWV phase-only, *every* cold start depends on FM RDS, and the owner's FM dial is marginal — the device could hear WWV clearly and still not know what time it was. The 100 Hz timecode chain (v2026.08.04.001: a second Goertzel bin at 100 Hz/50 ms beside the 1000 Hz marker bin, pulse-width symbols, two-frame corroborated decode) now provides date + absolute time from HF alone, and unseeded listen windows exist to use it. Two of the original premises deserve the record: the "below the audio passband" claim was never measured and is the first thing the field will now test (the `code[]` serial line and the About HF page carry the bin's floor/peak); and manual entry, also promised here, existed in no menu until the same build (Settings → Set Clock). The marker remains the *phase* instrument; the timecode is the *identity* instrument; a dateless marker still can never seed the clock.
